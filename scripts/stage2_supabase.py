"""Stage 2 — Supabase schema + RLS + clients.

Writes:
 - supabase/migrations/0001_init.sql   (tables, indexes, RLS, storage bucket)
 - lib/supabase/server.ts, browser.ts, admin.ts
 - lib/env.ts

Run: python scripts/stage2_supabase.py
Then apply the migration with one of:
  (a) supabase db push  (after `supabase link` to project ref vyaoxjqfzkkqpxydsbfd)
  (b) paste supabase/migrations/0001_init.sql into the Supabase SQL editor
"""
...
# ---------------------------------------------------------------- migration
MIGRATION = r"""-- VeriSight initial schema, RLS, and storage bucket.
...
write("supabase/migrations/0001_init.sql", MIGRATION)
...
print("\nStage 2 files written.")
print("Next: apply supabase/migrations/0001_init.sql")
